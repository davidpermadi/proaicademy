...
async function vaultGet(admin: any, name: string): Promise<string | null> { try { const { data } = await admin.rpc("get_app_secret", { p_name: name }); return data ?? null; } catch (_) { return null; } }

Deno.serve(async (req) => {
  if (req.method === "OPTIONS") return new Response("ok", { headers: cors });
  if (req.method !== "POST") return json({ error: "Method not allowed" }, 405);
  try {
    const SUPABASE_URL = Deno.env.get("SUPABASE_URL")!; const SERVICE_KEY = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!; const ANON_KEY = Deno.env.get("SUPABASE_ANON_KEY")!;
    ...
    const lang = String(body.lang ?? "").trim().toLowerCase() === "id" ? "id" : "en";
    if (!user && !isEmail(email)) return json({ error: "A valid email is required for guest checkout" }, 400);

    const lineItems: any[] = []; let gross = 0;
    for (const it of items) { const table = TABLE_FOR[it.product_type]; if (!table) continue; const { data: row } = await admin.from(table).select("*").eq("id", it.product_id).eq("is_published", true).maybeSingle(); if (!row) continue; const price = Number(row.price) || 0; if (price <= 0) continue; const qty = Math.max(1, parseInt(String(it.qty ?? 1)) || 1); const title = (row.title && row.title.en) || (row.name && row.name.en) || it.product_id; lineItems.push({ id: `${it.product_type}:${it.product_id}`.slice(0, 50), price, quantity: qty, name: String(title).slice(0, 50), product_type: it.product_type, product_id: it.product_id }); gross += price * qty; }
    ...
    const { data: order, error: oErr } = await admin.from("orders").insert({ user_id: user ? user.id : null, guest_email: user ? null : email, access_token: accessToken, midtrans_order_id: midOrderId, gross_amount: gross, status: "pending", customer_name: fullName || null, customer_phone: phone || null, customer_lang: lang }).select().single();
